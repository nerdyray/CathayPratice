...
describe('Create001', () => {
  let component: Create001; // 宣告要測試的元件實例
  let fixture: ComponentFixture<Create001>; // 宣告一個測試夾具，用來幫助我們與元件互動

  // `beforeEach` 會在每個 `it` 測試案例執行前都先執行一次
  beforeEach(async () => {
    // `TestBed` 是 Angular 測試的主要工具，可以建立一個模擬的 Angular 環境
    await TestBed.configureTestingModule({
      imports: [Create001] // 匯入要測試的獨立元件
    })
    .compileComponents(); // 編譯元件的樣板和樣式

    // 透過 TestBed 建立一個 Create001 元件的實例
